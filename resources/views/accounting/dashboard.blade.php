    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="text-xl font-bold text-[#0B4D2C] leading-tight">Transaction Dashboard</h2>
            </div>
            <div class="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto">
                ...
                <a href="{{ route('reports.transactions.pdf', ['from' => $from, 'to' => $to]) }}" class="inline-flex items-center justify-center rounded-lg border border-[#FF6B35] px-4 py-2 text-sm font-medium text-[#FF6B35]">
    ...
    <div class="space-y-6">
        ...
        <section class="stat-grid sm:grid-cols-2 xl:grid-cols-3">
            <article class="stat-card">
                ...
                <p class="stat-value">{{ 'K ' . number_format($monthlyIncome, 2) }}</p>
            </article>
            ...
            <article class="stat-card">
                ...
                <p class="stat-value">{{ 'K ' . number_format($monthlyDirectCosts, 2) }}</p>
            </article>

            <article class="stat-card stat-card-wide">
                <div class="stat-head">
                    <p class="stat-title">Operating Profit</p>
                    <span class="stat-icon {{ $monthlyOperatingProfit >= 0 ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700' }}">
                ...
                <p class="stat-value">{{ 'K ' . number_format($monthlyOperatingProfit, 2) }}</p>
            </article>
        </section>

        <section class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <p class="text-sm font-medium text-slate-600">Showing your totals and transactions from {{ \Carbon\Carbon::parse($from)->format('d M Y') }} to {{ \Carbon\Carbon::parse($to)->format('d M Y') }}.</p>
        </section>
        ...
        <section class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <div class="flex items-center justify-between border-b border-slate-200 px-4 py-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('reports.transactions') }}" class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">View Full Log</a>
                </div>
            </div>
            ...
            @if ($recentTransactions->isEmpty())
                <div class="px-6 py-12 text-center">
                    ...
                    <p class="mt-1 text-sm text-slate-500">Use Record Entry to add your first income or expense item.</p>
                </div>
            @else
                        ...
                        <tbody>
                            @foreach ($recentTransactions as $transaction)
                                <tr class="odd:bg-white even:bg-[#F4F7F5] hover:bg-slate-100/70" x-show="showAll || {{ $loop->index }} < 5">
                                    <td class="px-4 py-3 font-medium text-slate-700">{{ $transaction->date->format('d M Y') }}</td>
                                    <td class="cell-truncate px-4 py-3 font-medium text-slate-700">{{ $transaction->account?->code }} - {{ $transaction->account?->name }}</td>
                                    <td class="px-4 py-3">
                                        @if (($transaction->account?->type ?? '') === 'income')
                                            <span class="inline-flex rounded-full bg-green-100 px-2.5 py-1 text-xs font-medium text-green-700">Income</span>
                                        @elseif (($transaction->account?->type ?? '') === 'cogs')
                                            <span class="inline-flex rounded-full bg-orange-100 px-2.5 py-1 text-xs font-medium text-orange-700">COGS</span>
                                        @else
                                            <span class="inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700">{{ strtoupper($transaction->account?->type ?? 'n/a') }}</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 font-medium text-slate-900">{{ 'K ' . number_format($transaction->displayAmount(), 2) }}</td>
                                    <td class="px-4 py-3">
                                        @if ($transaction->payment_status === 'paid')
                                            <span class="inline-flex rounded-full bg-green-100 px-2.5 py-1 text-xs font-medium text-green-700">Paid</span>
                                        @elseif ($transaction->payment_status === 'partially_paid')
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('accounting.transactions.edit', $transaction) }}" class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50">Edit</a>
                                            <form method="POST" action="{{ route('accounting.transactions.destroy', $transaction) }}" data-confirm-title="Delete transaction?" data-confirm-message="This action cannot be undone." data-confirm-confirm-text="Delete" data-confirm-variant="danger">
                                                @csrf
                                                @method('DELETE')
                ...
                <x-recent-table-footer :total="$recentTransactions->count()" />
                </div>
            @endif
        </section>
    </div>
