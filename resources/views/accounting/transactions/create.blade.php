<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="text-xl font-bold text-[#0B4D2C] leading-tight">Record Financial Transaction</h2>
        </div>
    </x-slot>
    ...
    <div class="max-w-3xl space-y-4">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"
            x-data="{
                selectedType: @js(old('transaction_type', '')),
                selectedAccount: @js((string) old('account_id', '')),
                ...
                selectedSupplier: @js((string) old('supplier_id', '')),
                suppliers: @js($suppliers->map(fn($supplier) => [
                    'id' => (string) $supplier->id,
                    'name' => $supplier->name,
                ])->values()),
                selectedPaymentStatus: @js(old('payment_status', 'pending')),
                ...
                syncAccountSelection() {
                    ...
                    if (this.selectedType === 'debts') this.selectedPaymentStatus = 'pending';
                }
            }"
            x-init="syncAccountSelection()"
        >
            <form method="POST" action="{{ route('accounting.transactions.store') }}" class="space-y-4">
                @csrf
                <input type="hidden" name="payment_status" :value="selectedType === 'debts' ? 'pending' : selectedPaymentStatus">
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach ($transactionTypeOptions as $option)
                            <option value="{{ $option['value'] }}" @selected(old('transaction_type') === $option['value'])>{{ $option['label'] }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0.01" name="amount" value="{{ old('amount') }}" class="block w-full rounded-lg border-slate-300 text-sm" required>
                ...
                <div>
                    ...
                    <input type="date" name="date" value="{{ old('date', now()->toDateString()) }}" class="block w-full rounded-lg border-slate-300 text-sm" required>
                ...
                <div>
                    ...
                    <p x-show="selectedType === 'debts'" class="mt-1 text-xs text-slate-500">Debt entries start as Pending and update automatically when payments are recorded.</p>
                    @error('payment_status')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">Description</label>
                    <textarea name="description" rows="4" class="block w-full rounded-lg border-slate-300 text-sm">{{ old('description') }}</textarea>
                    @error('description')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
                </div>

                <div class="flex items-center gap-2">
                    <button type="submit" class="inline-flex items-center rounded-lg bg-[#32CD32] px-4 py-2 text-sm font-medium text-[#0B4D2C]">Save Entry</button>
                    <a href="{{ route('accounting.dashboard') }}" class="inline-flex items-center rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700">Cancel</a>
                </div>
            </form>
        </div>
    </div>
