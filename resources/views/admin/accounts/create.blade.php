    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="text-xl font-bold text-[#0B4D2C] leading-tight">Create Account</h2>
            </div>
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.accounts.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input name="code" value="{{ old('code') }}" class="w-full rounded-lg border-slate-300 text-sm" required>
                    @error('code')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">Name</label>
                    <input name="name" value="{{ old('name') }}" class="w-full rounded-lg border-slate-300 text-sm" required>
                ...
                <div class="flex items-center gap-2">
                    <button type="submit" class="inline-flex items-center rounded-lg bg-[#32CD32] px-4 py-2 text-sm font-medium text-[#0B4D2C]">Save Account</button>
