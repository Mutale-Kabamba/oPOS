    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 class="text-xl font-bold text-[#0B4D2C] leading-tight">Create Supplier</h2>
            </div>
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.suppliers.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input class="w-full rounded-lg border-slate-300 text-sm" name="contact_person" value="{{ old('contact_person') }}">
                </div>
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">Email</label>
                        <input type="email" class="w-full rounded-lg border-slate-300 text-sm" name="email" value="{{ old('email') }}">
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">Phone</label>
                        <input class="w-full rounded-lg border-slate-300 text-sm" name="phone" value="{{ old('phone') }}">
                    </div>
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">Address</label>
                    <input class="w-full rounded-lg border-slate-300 text-sm" name="address" value="{{ old('address') }}">
                </div>
                ...
                <div class="flex items-center gap-2">
                    <button type="submit" class="inline-flex items-center rounded-lg bg-[#32CD32] px-4 py-2 text-sm font-medium text-[#0B4D2C]">Save Supplier</button>
